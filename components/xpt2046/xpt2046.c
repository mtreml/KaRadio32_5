...
void saveCalibration(uint32_t x, uint32_t y)
{
	...
	ESP_LOGI(TAG, "Calibration saved: X:%" PRIu32 ", Y:%" PRIu32 "", x, y);
}
void getCalibration()
{
	...
	ESP_LOGI(TAG, "Calibration read: X:%" PRIu32 ", Y:%" PRIu32 "", tp_calx, tp_caly);
}
...
bool IRAM_ATTR xpt_read_touch(int *x, int *y, uint8_t raw)
{
	...
	*y = 0;
	...
	ESP_LOGV(TAG, "xleft:%d,xright:%d,ytop:%d,ybottom:%d,tp_calx:%" PRIu32 ",tp_caly:%" PRIu32 ",X:%" PRIu32 ",Y:%" PRIu32 "", xleft, xright, ytop, ybottom, tp_calx, tp_caly, X, Y);
	...
	ESP_LOGD(TAG, "Rotate: %d, X:%" PRIu32 ",Y:%" PRIu32 "", getRotat(), X, Y);
	*x = X;
	*y = Y;
	return true;
}
