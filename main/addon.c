...
void adcLoop()
{
	if (isAdcKeyboard)
	{
		...
		if (voltage < 985)
			ESP_LOGD(TAG, "Voltage: %" PRIu32 "", voltage);
		//			printf("VOLTAGE: %d\n",voltage);
...
bool irCustom(uint32_t evtir, bool repeat)
{
	...
	if (i < KEY_MAX)
	{
		...
		ESP_LOGV(TAG, "irCustom success, evtir %" PRIu32 ", i: %d", evtir, i);
		return true;
	}
	return false;
}
...
void irLoop()
{
	...
	while (xQueueReceive(event_ir, &evt, 0))
	{
		...
		ESP_LOGI(TAG, "IR event: Channel: %x, ADDR: %x, CMD: %x = %" PRIu32 ", REPEAT: %d", evt.channel, evt.addr, evt.cmd, evtir, evt.repeat_flag);
		...
		if (event_lcd != NULL)
			while (xQueueReceive(event_lcd, &evt, 0))
			{
				...
				case estation:
					...
					ESP_LOGD(TAG, "estation val: %" PRIu32 "", (uint32_t)evt.lline);
...
void task_addon(void *pvParams)
{
	...
	xTaskCreatePinnedToCore(rmt_nec_rx_task, "rmt_nec_rx_task", 2300, NULL, PRIO_RMT, &pxCreatedTask, CPU_RMT);
