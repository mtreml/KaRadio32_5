...
#endif
	ESP_LOGI(TAG, "VS1053 LFreq: %" PRIu32 "", freq);
...
#endif
	ESP_LOGI(TAG, "VS1053 HFreq: %" PRIu32 "", freq);
...
uint16_t VS1053_ReadRegister(uint8_t addressbyte)
{
	...
	ret = spi_device_transmit(vsspi, &t); // Transmit!
	if (ret != ESP_OK)
		ESP_LOGE(TAG, "err: %d, VS1053_ReadRegister(%d), read: %" PRIu32 "", ret, addressbyte, (uint32_t)*t.rx_data);
